...
export default function Login() {
  ...
  useEffect(() => {
    if (query.accessToken) {
      Cookies.set("token", query.accessToken);
  ...
  const LoginAllUsers = async (e) => {
    ...
    try {
      const newUser = { password: login.password, username: login.username };
      console.log({ newUser });
  ...
  //       `${process.env.REACT_APP_BASE_URL}/users/google`
  //     );
  ...
  return (
    <main className="w-full h-screen flex flex-col items-center justify-center px-4">
      <div className="max-w-lg w-full text-gray-600 space-y-5 shadow-2xl px-20 py-5">
